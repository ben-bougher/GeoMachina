...
;(function (window) {
  ...
  var scatter = function scatter(plot, data, primary, secondary){
    if(!plot){ return 'Param: plot is missing, a div to attach the svg is required'; }
  ...
  scatter.prototype.draw = function(){
    ...
    this._svg.selectAll('circle').data(this._data)
      .enter().append('circle')
      .attr('cx', function(d){
	return plot._xScale(d["x"]);})
      .attr('cy', function(d){
	return plot._yScale(d["y"]);})
      .attr('r', 2);
  ...
  scatter.prototype.reDraw = function(data, 
    ...
    if(xDomain){
      plot._xScale.domain(xDomain);
      plot._svg.select('.x.axis')
	.transition()
	.duration(plot._duration)
	.call(plot._xAxis)
	.ease('linear');
    }

    if(yDomain){
      plot._yScale.domain(yDomain);
      plot._svg.select('.y.axis')
	.transition()
	.duration(plot._duration)
	.call(plot._yAxis)
	.ease('linear');
